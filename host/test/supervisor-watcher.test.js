...
test('heal rule: while broken, node_modules / lockfile events pass; package.json → onInstall', async () => {
  ...
  await settle()
  assert.equal(changes.length, 1, 'the root node_modules entry passes while broken (same fingerprint, still a change)')
  fs.writeFileSync(path.join(dir, 'package.json'), '{"dependencies":{}}')
  await settle()
  assert.equal(installs.length, 1)
  assert.equal(changes.length, 2, 'lockfile/package.json passes as a change while broken too')
  broken = false
