# host/drill/joint — the §4.3 session pod (host/drill/step2 shape, unchanged) pointed at the REAL spine
# registrar (the `spine` Service in the same namespace). __IMAGE__, __NS__, __SPINE_URL__, __BOOTSTRAP__
# are substituted by remote.sh; ATELIER_BOOTSTRAP is the bootstrap the spine minted (serve.js
# `Registrar.bootstrapFor`), which the launcher writes to /run/atelier/bootstrap.token (0400) and the host
# exchanges at POST /v1/host/register. The code tree is staged by the `stage` init container.
apiVersion: v1
kind: Pod
metadata: { name: computer, namespace: __NS__, labels: { app: spike-step2-joint } }
spec:
  ...
  - name: session
    ...
    - { name: CHAT_ID, value: spike-step2-joint }
    - { name: PERSONA, value: bayard }
    - { name: PERSONA_TEXT, value: "You are a drill persona. Say nothing." }
    - { name: STORY_TEXT, value: "" }
    - { name: CHANNEL_URL, value: "__SPINE_URL__" }
    - { name: CHANNEL_TOKEN, value: canary-channel-token }
    - { name: CHANNEL_CHAT, value: spike-step2-joint }
    - { name: ATELIER_BOOTSTRAP, value: "__BOOTSTRAP__" }
