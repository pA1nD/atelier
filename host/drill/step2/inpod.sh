...
check "/work/apps/blitzfeed/backend.js (the agent's file, untouched)" "$(st /work/apps/blitzfeed/backend.js)" "1000:1000 644"
...
SOCK=$(ls $R/w/$IB/ | head -1); check "$R/w/$IB/$SOCK (after READY)" "$(st $R/w/$IB/$SOCK)" "0:0 700"
...
check "$R/session/dev.token" "$(st $R/session/dev.token)" "1000:1000 400"
...
check "agent reads its dev token" "$(as1000 wc -c < $R/session/dev.token)" "64"
