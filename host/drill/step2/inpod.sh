...
check "$A" "$(st $A)" "0:0 711"
check "$A/agent.log" "$(st $A/agent.log)" "0:1000 640"
check "$A/$IB (marker dir)" "$(st $A/$IB)" "0:0 711"
check "$A/$IB/slug" "$(st $A/$IB/slug)" "0:0 600"
check "$A/$IB/uid" "$(st $A/$IB/uid)" "0:0 600"
check "$A/$IB/revision.json" "$(st $A/$IB/revision.json)" "0:0 600"
...
check "$R/w/$IB (after READY: no worker write bit)" "$(st $R/w/$IB)" "0:$UB 710"
