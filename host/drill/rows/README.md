...
before the launcher (root makes `/work/apps` 0755 → chown 1000; uid 1000 copies two apps in with `.atelier-seeded` +
`.image-stamp` through a `.seeding` rename — the portal entrypoint's shape: `seedy` (`apps/seedy`, `hello` plus
`ctx.suspendable()` — the R14 declaration) and `steady` (`hello` as is: no declaration, a handle held — R14 leaves it
running, as it leaves the portal's `home` and `catalyst-chrome`, which declare nothing either). Why it exists (review 2026-09-02): B1 was
...
Rows: **9s-a** Ready ⇒ LIVE (S1) — the FIRST `/_atelier/apps` answer after the pod turns Ready already shows BOTH rows
prod-live at rev 1 with a 40-hex `deployed_rev`, no dev slot (built side by side inside the first scan); the host log has
`rev 1 LIVE (prod)` for each, agent.log `host: ready … seeded host: after the first scan`, never `module.json missing`,
never a `(dev)` line; **9s-b** the model is real and the road
...
(commit = `deployed_rev`, `at` a ms epoch), the fake spine saw the POST; **9s-e** R14 on the pod (S2): 70 s quiet → seedy `rev 1
STOPPED`, no worker process, the next request 200 + `RESUMED`, while steady stays live and answers; **9s-f** a re-seed over the kept `/work` (uid 1000 changes a
