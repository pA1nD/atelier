...
| `/work/.atelier/<inst>/` | `0:0 0711` | host at claim | markers, every one `0600` (the host's alone): `slug`, `uid`, `revision.json` (`{rev, live, sha256, bytes, builtAt, host, chrome, protocol, fingerprint, slug, prod:{rev, commit, deployedAt, message, legacy?, chrome?}}` — `chrome` = the chrome the DEV sheet was built with, `prod.chrome` the PROD sheet's; a deploy/rollback and a chrome rebuild stamp it, an adopt names none), `current` → `../last-good/<inst>/rev-N` (the PROD rev; symlink, atomic rename), `current-dev` → the DEV rev, `registered.json` (`{instance, slug, uid, company}`), `releases.jsonl` (the last 50 release rows), `backups.json` (backup sizes) |
| `/work/.atelier/chrome` | `0:0 0755` | host (`hostDirs`) | the chrome cache (step 7 ship C, `host/chrome/fetch.mjs`): `<digest>/` (dirs 0755, files 0644 — `frontend.js`, `kit.js`, `styles.css`, `chrome.css`, `fonts/*.woff2`, `manifest.json` last) fetched from the spine (`GET /v1/host/chrome/<digest>`) and verified (every sha, the recomputed digest), `current` → `<digest>` (symlink, atomic rename); current + previous kept, older pruned at a swap, `.tmp-*` swept at boot; root-owned under a root `0711` parent (nothing else can create, rename or unlink in it) and world-readable — these are the PUBLIC chrome bytes every company origin serves, and the dev shell serves them at `/_chrome/<digest>/…`; every app sheet compiles against `current` (ATELIER_CHROME_DIR is the fallback while no release is held) |
...
- The config hold: every spawn reads `registrar.appConfig` (§7) first, and the door's answer decides whether the spawn
  goes. A 2xx with a whole document is the composed document, cached in memory as the row's last-known one for this host
  life (never to disk); a 404 (no config rows) is the empty document, known from then on. The door FAILING — a 5xx, API
  50's `503 no config key`, a network error, a timeout — WITH a last-known document spawns on it (`slot.configStale`)
  and WITHOUT one HOLDS the spawn. A MASKED document — a 200 whose `sealed_missing` is non-empty (API 50: keys the spine
  could not unseal, kept out of `env`) — is no document at all: it never becomes the last-known one, and the spawn is
  HELD even with a cache (the spine's contract: the host refuses the spawn while the list is non-empty — the cache holds
  the old plaintext of the very keys the spine can no longer open, and a rotated key's value must not reach a fresh
  worker); a worker already running keeps the document it holds. HELD (`{error:'config-held'}` from `workerSpec`) = no
  worker — never one without its env — the slot `loading` (a request answers 503 `app not ready`), the built rev dropped,
  no report, no save verdict. One log line per row per reason, the keys named and never a value. Every `scan()` retries a
  held or stale row with ONE read first (never a rebuild against a closed door): held → the spawn again (dev rebuild,
  prod resume, the seeded build); stale → the fresh document compared with the one the worker runs on — prod: a config
  release (D16) when it moved, nothing when it did not; dev: the worker stopped when it moved, the next request resumes
  it on the fresh document (a running worker's env cannot be swapped); an idle-stopped worker simply reads fresh at its
  next resume. A config stamp (D16) reads the door before it acts: a failed read leaves the running workers on their
  document, marked stale for the scan. The install specs read no document (`config:false`); the deploy road reports
  `config-held` as the failing rehearsal step.
...
every sha and the recomputed digest (a bundle already on disk is verified the same way before it is
swapped to, and `current` is verified at boot — a corrupt dir is removed and fetched again), swaps
`current` and — once the supervisor has booted — `supervisor.rebuildAll()`: every DEPLOYED prod slot
whose sheet is not built against the held chrome (`revision.json.prod.chrome`) gets a NEW rev of the same
code with a sheet compiled against it (`store.clone`, no gate, no restart; `current` moves, the previous
rev stays addressable for the window, one `onSwap` → `modulesChanged` → the company's frames, so a tab on
that app reloads once with the new chrome), every linked row one dev rebuild per chrome. **The heartbeat
reports the digest every prod sheet is BUILT with** (`chrome.built()`), never the one merely held: the
shell composes an app document's JS from the reported digest and its CSS is the sheet the host built, so
the two must be one digest. Hence the rebuild is all or nothing — phase 1 compiles every sheet that is
behind (nothing moves), phase 2 commits them all; one sheet refused (a css failure, a row mid-deploy, the
host in fault) and nothing commits, the computer keeps reporting the previous digest, and the next beat
that names the held digest (`onHold`) tries again — idempotent and cheap when nothing is behind, so a
row that falls behind later (adopted, rolled back, its deploy over) is caught within a beat. A css
failure is logged once per (chrome, reason), not once per beat. `row.deploying` is held for a row's
commit alone (milliseconds); a deploy landing inside it answers 409 `deploy in progress`. The new rev
moves the collector's running rev (`onSwap` → `setRunning`), which closes that app's open error tallies:
an error still firing after a chrome release notifies once more; the prod worker keeps running and its
reports carry the SLOT's rev (the rev it was born with would be `stale-rev` at the collector and the
spine). With a digest held the sheet build rewrites every RELATIVE `url()` of the chrome's source to
`/_chrome/<digest>/…` (`buildSheet({chromeBase})` — the bundle's `fonts/…` are served by the shell there;
the chrome's `styles.css` is one file: an `@import`ed sheet of its own is not rebased, and an app's own
`url()` candidates are never touched). A fetch that fails or is refused keeps `current` (the cached
fallback) and retries at the next beat; no cache and no folder = no chrome dir, as before a release. The
dev shell serves the same dir (`chrome.dir()` — the held one, ahead of the report) and the cache's files at
...
|---|---|---|
| `register()` | `POST /v1/host/register` | `{pod_ip, host_started_at}` → `{host_id, epoch, token, company, origin, chat, principal:{id,name}, apps:[{instance, slug, uid, rev, deployed_rev}], shell_public_key_hex, chrome, sleep}` — the previous epoch is revoked here; `chrome` = `{digest, version}` the computer's effective release, null while the spine names none (§6.4); `sleep` = `"24h" \| "always-on"`, the computer's sleep mode (API 50) → `registrar.sleep` (§6.1; `24h` before any answer, an absent or shapeless field keeps the last word) |
| `heartbeat(b)` | `POST /v1/host/heartbeat` | `{visible_apps, last_served_at, pod_ip, chrome_digest, resources?}` → `{ok, config:[{instance, updated}], chrome, sleep}` — `chrome_digest` = the digest the host holds (null = none); `resources` = `{cpu: 0..1, ram:{used,total}, disk:{used,total}}` in bytes (§6.5), present only when the host could read it; `chrome` and `sleep` as on register |
...
| `appConfig(instance)` | `GET /v1/apps/<instance>/config` | → `{env:{K:V}, sealed_missing:[K]}` (OR14, API 50; fetched at every worker spawn, cached in memory as the row's last-known document for the host life, never to disk) — a non-empty `sealed_missing` names keys the spine could not unseal, kept out of `env`: a masked document, the spawn held (§6.1 "the config hold") · `503 no config key` while the spine holds none · `404` = no config rows, the empty document |
...
10. **Chrome source** — a folder (`ATELIER_CHROME_DIR`), or the release the spine names, fetched by
    digest into `/work/.atelier/chrome` (step 7 ship C, §6.4). No chrome → app-less documents and
    pass-through app CSS.
...
`unknown app`, 409 `deploy in progress` — also for the milliseconds a chrome swap commits the row's rebuilt sheet (§6.4), 400 no message), answered as an NDJSON stream of `{t:'step', name, ms, ok,
