...
**Repeat loads stopped re-downloading megabytes.** Assets carried no caching headers at all — every page load re-sent the full client bundle, React, the chrome bundle, and the stylesheet (~14 MB in development on a real instance). Every built asset now carries an mtime-keyed `ETag` with `Cache-Control: no-cache` (revalidate every time): unchanged assets answer `304` with zero body, an edited file's new mtime is a new ETag so fresh bytes ship immediately — hot-reload freshness is untouched, the wire cost of a repeat load collapses to a few KB.

**Client errors stopped vanishing into tabs.** On `observe` instances every page installs a small reporter: uncaught errors, unhandled rejections, and `console.error` calls (React dev warnings never throw — this lane is where they live; first occurrence per distinct message per page, so a warning repeated every render can't eat the budget) POST to `/_atelier/client-errors` (throttled to 10/min per page so an error loop can't become a request storm) and the shell fans them out as `{type: 'client-error', error }` frames on the shell WS topic — stored nowhere in the shell, collected by whatever diagnostics module listens. Delivery follows the `backend-error` precedent: ACL-gated by the page's module, so a stack never reaches a client that can't see that module. Off (the default) → no listener installed, endpoint 404s.

**`byRoute` — traffic attributed per module.** Next to `total`, the observe layer's `/_atelier/inflight` now carries requests-since-boot bucketed by who each request was for (`api/<ws>/<id>`, `modules/<ws>/<id>`, `assets`, `_atelier`, `other`), sorted busiest-first and bounded (past 300 buckets new ones fold into `other`). Differenced per tick it answers "which module is producing the traffic". Flag-gated with the rest of the observe layer; zero cost when off.
