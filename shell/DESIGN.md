...
| **the waking page** | none (502/ECONNREFUSED = a broken page) | a `503 {waking:true}` on a bundle import, a snapshot or an API call the client itself makes (`/_atelier/*`) → the waking panel inside the chrome (`active.kind: 'waking'`; the plain `<pre>` only while no chrome renders) with a 2 s → 5 s backoff poll of `/_atelier/wake?company=<c>[&app=<active slug>]` (the shell wakes the computer on the first miss, §3.5), bounded at 60 s like the shell's page (`onGiveUp` → the copy says the wake is taking unusually long, reload by hand); on `{ok:true}` → `location.reload()`. App fetches are the app's own; the fallback covers the shell's |
