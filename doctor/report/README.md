...
| `merge.mjs` | `mergeModule(...)` → report.json. Static findings normalised; probe observations → findings through `OBSERVATIONS` (kind → rule, the seed report.mjs lambdas: an egress is N4/N5/I2, an env read N2/N2op/N3/D13, a write outside dataDir N1/D13, listens D2, spawns D12 unless in `IMAGE_BINS`, signals/exit N8; state ≠ mounted → R1, resources → R2, no teardown / killed → R3). Cells: `rule.count(static, probe, tw)` when the catalogue has one, else `static.cells[id]`, else the R1–R3 fallbacks. `configKeys` = names only |
...
| `write.mjs` | `<out>/doctor/<module>/{report.json, module.json, config-keys.json, rewrite/<rel>}`, `<out>/doctor/{portability.csv, rows.md, modules.md, summary.json, verdict.txt}`; `applyWrite` (--write: git work tree, no uncommitted change to the touched files, an existing module.json only replaced when its sole change is the N11 key drop); `outInside` |
