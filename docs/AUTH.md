...
| `/api/<ws>/<id>/*` — module API | `authenticate` → **presence** → **`authorize`** | Exempt from presence + authorize: the **auth module's own routes** (can't gate the gate) and the **active chrome** (resolved server-side; in nobody's `workspaces`, but shared scaffolding — e.g. its `/docs`). A feature module **can't** self-exempt via `meta`. |
...
Both layers exempt two things: (a) the configured **auth module's own** qualifiedId — you can't gate the gate (login must be reachable), and it governs its own surface through `authenticate`; and (b) the **active chrome** — resolved server-side via `resolveChromeQid`, **never** trusted from a module's self-declared `meta`. It's excluded from `buildDefaultUser`, so it's in nobody's `user.workspaces` and the presence gate would `403` it for everyone, but the chrome (and its API, e.g. `/docs`) is shared scaffolding every authed user loads, not tenant data. A feature module can't opt its own API out of the gate by exporting `meta.hidden`/`meta.chrome` — only the one resolved chrome qid is waved through. With no auth module the user is the full-access `defaultUser`, so the presence gate is a no-op and `authorize` is never called.
