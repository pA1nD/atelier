...
`meta` is optional; the rail falls back to `icon: 'square'` and `name: <dir>`. It's read server-side at discovery — **from source, as a top-level object literal** (the shell does *not* run your `frontend.jsx` to get it) — and shipped in the HTML bootstrap, so grouping renders on first paint with no flicker. A computed `meta` (spread/imported value) falls back to *executing* the module in Node, where browser globals like `document` are undefined — so keep `frontend.jsx` top-level code side-effect-free.
