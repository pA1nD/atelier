...
- **Your `.gitignore` shapes the cut.** When the module's working tree is a git repo, a cut ships what that repo tracks plus untracked files its in-tree `.gitignore` rules don't exclude — build output you ignored never travels (an ignored file you track anyway still ships: tracking it means you meant it). Global excludes are not consulted, so cuts stay machine-independent; modules not under git cut unfiltered. The packager report prints each cut's file count and the delta vs. the previous cut, so a snapshot that suddenly balloons is visible.
- **`data/` is runtime state.** It ships only when a cut says `--data` (point-in-time content, placed on first install; shipped even if the module's `.gitignore` ignores `data/` — the flag is the explicit request); a `--force` reinstall always keeps the live `data/`.
