...
**Across all of them, a module is _untrusted_.** It can't reach another tenant's data or exempt itself from the gate — the shell enforces the boundary from `user.workspaces`, never from a module's self-declared `meta`. **Why good:** a feature can be vibe-coded (or agent-coded) freely without ever becoming a security or isolation hole.
