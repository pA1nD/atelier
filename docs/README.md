...
| `marketplaces` | `[]` | — | github `owner/repo` repos the [`atelier add`](./INSTALL.md#marketplaces) installer resolves bare module names against (register with `atelier add --marketplace`). Tooling-only — the server ignores it |
